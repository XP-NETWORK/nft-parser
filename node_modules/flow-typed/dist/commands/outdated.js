...
async function run(args) {
  ...
  const cachedLibDefs = await (0, _npmLibDefs.getCacheNpmLibDefs)(Number(args.useCacheUntil) || _cacheRepoUtils.CACHE_REPO_EXPIRY, true);
  const installedLibDefs = await (0, _npmLibDefs.getInstalledNpmLibDefs)(flowProjectRoot, args.libdefDir ? String(args.libdefDir) : undefined);
  let outdatedList = [];
    ...
    await Promise.all([...installedLibDefs.values()].map(async installedDef => {
      ...
      if (installedDef.kind === 'LibDef' && installedDef.libDef.name === cachedDef.name && installedDef.libDef.scope === cachedDef.scope) {
        ...
        if (npmLibDef) {
          ...
          const file = await _node.fs.readFile(_path.default.join(cwd, installedDef.libDef.path), 'utf8');
