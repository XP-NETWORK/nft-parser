...
const moralis = async (address: string, tokenId: string, chain: any) => {
    try {
        ...
        if (
            ...
            response.jsonResponse?.token_uri
        ) {
            try {
                let uri = response.jsonResponse?.token_uri;
                const spl = uri.split("/");
                // console.log(uri.replace(spl[5].split(".")[0], String(Number(spl[5].split(".")[0]))))
                const newUri = uri.replace(
                    spl[5].split(".")[0],
                    String(Number(spl[5].split(".")[0]))
                );
...
export const divineAnarchy = async (
    nft: any,
    account: string,
    whitelisted: boolean
) => {
    ...
    try {
        ...
        const image = data.image.replace(
            "https://divineanarchy.mypinata.cloud/",
            "https://ipfs.io/"
        );
        console.log({ image });
